<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Batch Stok: ') . $productBatch->batch_number }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow sm:rounded-lg p-6">
                ...
                <form action="{{ route('product-batches.update', $productBatch->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="product_id" class="shadow-sm appearance-none border @error('product_id') border-red-500 @else border-gray-300 @enderror rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-blue-500">
                            ...
                            @foreach($products as $product)
                                <!-- Menggunakan $productBatch->product_id sebagai nilai bawaan -->
                                <option value="{{ $product->id }}" {{ old('product_id', $productBatch->product_id) == $product->id ? 'selected' : '' }}>
                                    {{ $product->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="batch_number" value="{{ old('batch_number', $productBatch->batch_number) }}" 
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="qty" value="{{ old('qty', $productBatch->qty) }}" 
                    ...
                    <div class="mb-6">
                        ...
                        <input type="date" name="expired_date" value="{{ old('expired_date', $productBatch->expired_date) }}" 
